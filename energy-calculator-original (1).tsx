...
const EnergyCalculator = () => {
  ...
  const [heatingSystem, setHeatingSystem] = useState('Oil Furnace');
  const [coolingSystem, setCoolingSystem] = useState('Central AC');
  const [electricityRate, setElectricityRate] = useState(DEFAULT_RATES.electricity);
  const [oilRate, setOilRate] = useState(DEFAULT_RATES.oil);
  const [gasRate, setGasRate] = useState(DEFAULT_RATES.gas);
  const [currentSEER, setCurrentSEER] = useState(10);
  const [useManualInput, setUseManualInput] = useState(false);
  const [manualHeatingCost, setManualHeatingCost] = useState(0);
  const [manualCoolingCost, setManualCoolingCost] = useState(0);
  
  // State for calculated values
  const [currentHeatingCost, setCurrentHeatingCost] = useState(0);
  const [currentCoolingCost, setCurrentCoolingCost] = useState(0);
  const [heatPumpHeatingCost, setHeatPumpHeatingCost] = useState(0);
  const [heatPumpCoolingCost, setHeatPumpCoolingCost] = useState(0);
  const [monthlySavings, setMonthlySavings] = useState({ heating: 0, cooling: 0, total: 0 });
  const [annualSavings, setAnnualSavings] = useState({ heating: 0, cooling: 0, total: 0 });
  ...
  const updateCosts = (currentHeating, currentCooling, heatPumpHeating, heatPumpCooling) => {
    // Annual costs
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <YAxis tickFormatter={(value) => `$${value}`} stroke="#999999" />
                <Tooltip 
                  formatter={(value) => [`$${Math.round(value)}`, 'Savings']}
                  contentStyle={{ backgroundColor: '#333', borderColor: '#444' }}
                  labelStyle={{ color: THEME.primary }}
                  itemStyle={{ color: '#fff' }}
                />
